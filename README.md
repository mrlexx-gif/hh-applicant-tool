...
> [!IMPORTANT]
> Ищу почасовую или проектную [@feedback_s3rgeym_bot](https://t.me/feedback_s3rgeym_bot) (Python, Vue.js, DevOps). Не пишите туда с вопросами по утилите. Для них есть канал!!! Так же буду благодарен, если посоветуете меня в качестве сись-пись админа, погромиста и дквжупса своим знакомым, начальству... Имею огромный опыт фриланса (это может и не нравится тупопездым идиотинам, мнящим себя дохуя специалистками по подбору), но наебизнес ценит тех, кто умеет решать проблемы быстро, не задавая лишние вопросы и без надзирателей... 
...
> [!NOTE]
> Настоящие погромисты пишут только в <s>блокноте</s> <s>vim</s> neovim. ЗАЦЕНИТЕ МОЙ [КОНФИГ NEOVIM](https://github.com/s3rgeym/nvim-config) НА КОТОРОМ Я ФАКТИЧЕСКИ РЕДАКТИРУЮ ВЕСЬ КОД, УДАЛИВ ДАВНО VS CODE И ПР ЗАЛУПУ!!!
...
> [!WARNING]
> Нет НЕЙРОХУЙНЕ!!! Прекратите слать код, который сами не понимаете. Мне лень просматривать высеры нейродурака, поэтому скопилось приличное количество нейрошлака... В 2026 программисты умерли, остались лишь операторы промптов... из-за которых даже я не могу найти никакую работу, так как любая чмоха возомнила себя погромистом, если в состоянии составить запрос к нейродураку, но тут скорее рынок сдох, да и овчар-шаболды сменили алгоритм отбора с активного на пассивный
...
> Авторские права на изображение принадлежат Кандинскому... нет, не человеку, а боту от сбера
...
**BTC Address:** `BC1QWQXZX6D5Q0J5QVGH2VYXTFXX9Y6EPPGCW3REHS` (спасибо за донаты — это единственное, что двигает разработку, так что не стесняйтесь и шлите их чаще, вы ЭКОНОМИТЕ КУЧУ ДЕНЕГ с учетом жадности владельцев сервисов для откликов)
...
- 🧠 **AI-фильтрация вакансий.** Можно автоматически фильтровать вакансии через AI, чтобы откликаться только на подходящие. Три режима: `heavy` — полный анализ резюме и вакансии (должность, навыки, опыт работы, задачи), `light` — быстрая оценка по названию и ключевым навыкам, `custom` — свой системный промпт.
...
- [HH Applicant Tool](#hh-applicant-tool)
  ...
  - [Решение тестов при отклике](#решение-тестов-при-отклике)
...
`custom` - полный контекст вакансии и резюме, но со своим системным промптом через `--ai-filter-prompt` (резюме добавляется в конец промпта блоком «Кандидат»)
...
# С собственным системным промптом
hh-applicant-tool apply-vacancies --ai-filter custom --ai-filter-prompt "Откликайся только на вакансии для Python-разработчика"
```
...
- `--ai-filter` — режим фильтрации: `heavy`, `light` или `custom`
- `--ai-rate-limit` — ограничение запросов к AI в минуту (по умолчанию 40)
- `--ai-filter-prompt` — системный промпт для AI-фильтра. Используется только в режиме `custom`
...
Таймауты задаются полями `timeout` (весь запрос: соединение и чтение ответа, по умолчанию 30 секунд) и `connect_timeout` (только соединение, по умолчанию 5 секунд). Их можно указать в любой из секций выше, сразу для всех — в секции `openai`, либо флагами `--openai-timeout` и `--openai-connect-timeout`. Если модель отвечает медленно, увеличивайте `timeout`.
...
## Решение тестов при отклике

На некоторые вакансии HeadHunter требует пройти тест перед откликом. В REST API нет
отдельного метода для решения тестов, поэтому утилита выполняет их через веб-версию
сайта: открывает страницу отклика, разбирает встроенный JSON с вопросами
и отправляет ответы обратно веб-формой.

Ответы на тест генерируются двумя способами:

- **Быстрый (по умолчанию):** без AI. Выбирается вариант «Да», если он есть, иначе —
  средний вариант (надеемся что правильный ответ находится посередине).
  Открытые вопросы без ссылок отвечаются «Да», со ссылками — отказным текстом.
- **Через AI:** при запуске с `--use-ai` и настроенной секцией `openai_cover_letter`
  каждый вопрос отправляется моделью, которая возвращает ID правильного ответа.

Управление поведением:

- `--skip-tests` — пропускать вакансии с тестами и не решать их вовсе.
- `--dry-run` — «решить» тест виртуально, без реальной отправки.

Если тест не удалось разобрать, утилита откатывается к обычному отклику через API.

### Пользовательский контекст для AI (`--context-dir`)

Чтобы ответы AI на открытые вопросы тестов (а также сопроводительные письма)
были персонализированными, укажите директорию с файлами контекста:

```
hh-applicant-tool apply --use-ai --context-dir ./context
```

Все файлы `*.md` из этой директории загружаются и добавляются в системный промпт
AI. Пример содержимого `./context/profile.md`:

```markdown
# Мой профиль
ФИО: Алексей
Должность: руководитель ИТ-проектов
Формат работы: удалённый / гибридный
Зарплатные ожидания: от 300 000 ₽
Навыки: управление проектами, ЦОД, бюджетирование
```

Благодаря этому AI знает ваши данные: на вопрос «Какой формат работы вы
рассматриваете?» или «На какой уровень дохода ориентируетесь?» ответит исходя
из вашего профиля, а не общими фразами. Флаг поддерживается командами `apply`
и `reply-employers`. Если директория не указана или не найдена, поведение
не меняется (контекст не добавляется).
...
- **`%(vacancy_url)s`**: Ссылка на вакансию.
...
Через конфиг можно задать дополнительные настройки. Значения вложенных полей
указываются через точку (например, `config --set smtp.host smtp.yandex.ru`):
...
| `token`                 | Объект с OAuth-токенами: `access_token`, `refresh_token`, `access_expires_at`.             |
| `openai.proxy_url`      | Отдельный прокси только для запросов к AI. Переопределяется флагом `--openai-proxy`; если не задан — используется общий прокси |
| `openai.timeout`        | Таймаут запроса к AI: соединение и чтение ответа. Переопределяется флагом `--openai-timeout`, по умолчанию 30 секунд |
| `openai.connect_timeout`| Таймаут соединения с AI. Переопределяется флагом `--openai-connect-timeout`, по умолчанию 5 секунд |
| `smtp.host`             | SMTP-сервер для отправки писем при отклике (обязателен вместе с `port`)                    |
| `smtp.port`             | Порт SMTP-сервера (обязателен вместе с `host`)                                             |
| `smtp.user`             | Логин SMTP (используется для авторизации и как `From`, если `smtp.from` не задан)          |
| `smtp.password`         | Пароль SMTP                                                                                |
| `smtp.ssl`              | Использовать `SMTP_SSL` (по умолчанию `false`)                                             |
| `smtp.starttls`         | Выполнять `STARTTLS` при незашифрованном подключении (по умолчанию `true`)                 |
| `smtp.from`             | Заголовок `From` письма; если не задан — берётся из `smtp.user`                            |
| `apply_mail_subject`    | Шаблон темы письма при флаге `--send-email`, см. формат сообщений                          |
| `apply_mail_body`       | Шаблон тела письма при флаге `--send-email`, см. формат сообщений                          |

Настройки секций AI (`openai_cover_letter`, `openai_vacancy_filter`, `openai_captcha`) —
см. раздел «Использование AI».
...
Утилита читает следующие переменные окружения:

| Переменная                  | Описание                                                                                                             |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `CONFIG_DIR`                | Каталог, в котором хранятся данные приложения, вместо пути по умолчанию (см. «Данные приложения»). В Docker задаётся как `/app/config` |
| `HH_PROFILE_ID`             | Имя подкаталога-профиля внутри `CONFIG_DIR` (по умолчанию `.`). Позволяет держать несколько изолированных профилей    |
| `HTTP_PROXY` / `http_proxy` | Прокси для HTTP-запросов к API HH, если не задан `proxy_url` в конфиге                                              |
| `HTTPS_PROXY` / `https_proxy` | Прокси для HTTPS-запросов к API HH; если не задан — используется HTTP-прокси                                       |
| `PLAYWRIGHT_BROWSERS_PATH`    | Каталог, где Playwright хранит браузеры (например, при установке `playwright install chromium`). По умолчанию `~/.cache/ms-playwright` |

---

## Ответы на заебавшие вопросы

> Утилита перестала откликаться на тесты ряяяяяяяяяяяяяя

- Обычно это происходит, когда истекают куки и слетает авторизация на самом сайте (вы долго не запускали отклики). При этом команды типа `whoami` могут продолжать работать. Нужно по новой авторизоваться.

> Ряяяя поиск возвращает не то, что я и ищу, утилита не работает

- Сайт хуй-хуй, созданный для овчарок, и поэтому имеющий красный логотип цвета менстуральных выделений, зарабатывает в тч на услугах по продвижению вакансий, когда вакансию показать некому, то он ее начинает подсовывать калдидатам, которых это "может" заинтересовать (хоть кому-то показать надо), так программистам предлагают вакансии учителей и наставников в онлайн-школах и прочую дичь. Ничего с этим не сделать кроме применения дополнительных фильтров для отсеивания поискового мусора...

<img width="792" height="198" alt="image" src="https://github.com/user-attachments/assets/c532afaf-8cde-478b-8bc2-63955133f502" />

- Как работало, так и работает. Меньше читайте всяких пиздаболов
...
* `git clone ...`
* `poetry install .`
* **poetry** можно установить через **pipx**
* в дальнейшем можно запускать утилиту так `poetry run hh-applicant-tool authorize`
* либо можно активировать виртуальное окружение, которое создается в каталоге проекта (`source .venv/bin/activate`)
* после изменений ничего переустанавливать не надо (`poetry install` устанавливает проект в режиме редактирования, те все изменения в каталоге проекта "сразу применяются")
 
Для публикации новой версиии утилиты на `PyPi` нужно указывать версию. Она указывается в `pyproject.toml`. Когда делаете `Pull Request`, то увеличивайте эту версию на единичку (при малозначительных изменениях — последнее число).
